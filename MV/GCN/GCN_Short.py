            ...
            for epoch in range(1, Config.MAX_EPOCHS + 1):
                ...
                if epoch % 5 == 0 and not np.isnan(loss):
                    ...
                    if val_metrics['auc'] > current_best_val_auc:
                        ...
                        epochs_no_improve = 0
                        best_epoch = epoch
                        print(f"[Checkpoint] New best validation AUC: {current_best_val_auc:.6f} at epoch {epoch}")
                    else:
                        epochs_no_improve += 1
                        print(f"[Checkpoint] No improvement for {epochs_no_improve} evaluations (best at epoch {best_epoch})")
                        
                        if epochs_no_improve >= Config.PATIENCE // 5:  # Convert patience to evaluation cycles
                            print(f"[Checkpoint] Early stopping after {epochs_no_improve} evaluations without improvement")
                            break
