    ...
    def __init__(self, input_dim, embedding_dim, hidden_dim, num_nodes, num_layers, heads=8, concat=True, dropout_rate=Config.DROPOUT_RATE):
        ...
        for i in range(num_layers):
            ...
            if i == 0:
                ...
                # Previous layer output dimension depends on whether we concatenated heads
                in_channels = hidden_dim * heads if concat else hidden_dim
                
            # Output channels for GAT layer (before head multiplication)
        ...
        # Final linear output layer - dimension after last GAT layer
...
class Experiment:
    ...
    def create_config_name(self, config):
        ...
        config_name = (
            ...
            f"g{gamma}_"
            f"heads{heads}_"
            f"{'cat' if concat else 'avg'}"
        )
        
        return config_name

    def delete_all_previous_runs(self, project_name="gat-long"):
    ...
    def run_hyperparameter_tuning(self):
        ...
        self.delete_all_previous_runs("gat-long")
        
        # Initialize WandB sweep
        sweep_id = wandb.sweep(sweep=SWEEP_CONFIG, project="gat-long")
    ...
    def train_model(self):
        ...
        print(f"[Checkpoint] alpha={config.focal_alpha}, gamma={config.focal_gamma}, heads={config.heads}, concat={config.concat}")
        ...
        try:
            ...
            trainer = GATTrainer(model, Config.DEVICE, alpha=config.focal_alpha, gamma=config.focal_gamma)
